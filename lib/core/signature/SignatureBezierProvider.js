...
class SignatureBezierProvider extends EventEmitter {
  ...
  addPointToSignature(point, isEndOfLine = false) {
    if (this.isFirstPoint) {
      ...
      if (
        vec2.length(previousPoint, point) <
        SignatureBezierProvider.touchDistanceThreshold
      ) {
        return;
      }
  ...
  finalizeBezier(point3rd) {
    ...
    this.points[3].weight = SignatureBezierProvider.signatureWeightForLine(
      point2nd,
      pointAvg
    );
  ...
  addListener(event, fn, context) {
    ...
    return {
      remove: () => this.removeListener(event, fn, context),
    };
  }
}
