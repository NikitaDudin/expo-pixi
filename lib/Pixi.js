...
class ExpoPIXIApplication extends PIXI.Application {
  constructor({
    width, height, scale, backgroundColor, context, ...props
  }) {
    if (!context) {
      throw new Error(
...
const hasSizes = (asset) => (typeof asset.width === 'number' && typeof asset.height === 'number');

const requestImageSizes = async (uri) => new Promise((resolve) => {
  Image.getSize(
    uri,
    (width, height) => {
...
const getTextureAssetAsync = async (resource) => {
  ...
  if (Platform.OS !== 'web') {
    ...
    if (isImage(asset.localUri || asset.uri) && !hasSizes(asset)) {
      const { width, height } = await requestImageSizes(asset.localUri || asset.uri);
      asset.width = width;
      asset.height = height;
    }
  }
  ...
  PIXI.BaseTexture.from = (...props) => {
    ...
    if (props.length && props[0]) {
      ...
      if (isAsset(asset)) {
        ...
        return originalBaseTextureFrom(new HTMLImageElement(asset), ...props.slice(1));
  ...
  PIXI.Texture.from = (...props) => {
    ...
    if (props.length && props[0]) {
      ...
      if (isAsset(asset)) {
        ...
        return originalTextureFrom(new HTMLImageElement(asset), ...props.slice(1));
      } if (typeof asset === 'string' || typeof asset === 'number') {
        console.warn(
